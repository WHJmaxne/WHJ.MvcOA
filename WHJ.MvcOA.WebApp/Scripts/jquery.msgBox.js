...
(function (jqObj) {
    ...
    function MsgBox(s) {
        ...
        function readyMsgBox() {
            ...
            if (s != null) {
                ...
                if (s.waitImg != null) paras.waitImg = s.waitImg;
                if (s.bgImg != null) paras.bgImg = s.bgImg;
            }
        ...
        this.showMsgOk = function (msg, callBack) {
            if (arguments.length > 1) paras.callBack = arguments[1];
            showSysMsg(msg, 2);
        }
        ...
        //--------------
        this.showReqErr = function () { this.showMsgErr("请求错误 !"); }
        this.showReqOk = function () { this.showMsgOk("操作成功 !"); }
        this.showReqVF = function () { this.showSysMsgWTime("会话过期,3秒后自动返回登录界面 !", 1, 3000); }
        //显示ajax等待消息
        this.showWait = function () { this.showMsgWait("请稍后..."); }
        //-------------
    ...
    jqObj(document).ready(function () {
        jqObj.extend(jqObj, {
            msgBoxObj: new MsgBox({ imghref: "/Content/images/" })
        });
    });
})($);